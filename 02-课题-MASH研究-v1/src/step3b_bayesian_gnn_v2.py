# -*- coding: utf-8 -*-
"""
Step 3b (v2): Bayesian GNN for FXR pIC50 — improved training.
Fixes vs v1: (1) target standardization for NLL stability, (2) early stopping on
MC-mean validation RMSE (not single noisy pass), (3) 5-seed ensemble, (4) dual
evaluation protocol: STRICT scaffold split + proposal-style random split,
(5) pooled val+test UQ correlation.
"""
...
BOND_TYPES = [str(t) for t in [Chem.rdchem.BondType.SINGLE, Chem.rdchem.BondType.DOUBLE,
...
class BayesianGIN(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch):
        for conv, bn in zip(self.convs, self.bns):
            ...
            x = F.dropout(x, p=self.dropout, training=True)  # MC-dropout active at inference
...
def train_once(train_g, val_g, seed, epochs=500, lr=5e-4, patience=60):
    ...
    va_mc = DataLoader(val_g, batch_size=128, shuffle=False)
...
def run_experiment(name, split_fn):
    ...
    for seed in [42, 7, 2024, 11, 99]:
        m, bv, (ym, ys) = train_once(tr_g, va_g, seed)
        models.append((m, (ym, ys)))
        stats.append(bv)
    print("best val RMSE per seed:", [round(s, 3) for s in stats])
    ...
    print(f"[BGNN test] R2={r2:.3f} RMSE={rmse:.3f} MAE={mae:.3f} Spearman={sp:.3f}")
    print(f"[BGNN UQ pooled val+test n={len(y_pv)}] Spearman(sig,|err|)={sp_uq:.3f} "
          f"cov50/68/95={cov[0]:.2f}/{cov[1]:.2f}/{cov[2]:.2f} ECE={ece:.3f}")
    ...
    for nm, mdl in [("RF", RandomForestRegressor(n_estimators=800, random_state=SEED, n_jobs=-1)),
        ...
        print(f"[{nm} test] " + " ".join(f"{k}={v:.3f}" for k, v in base[nm].items()))
    ...
    return dict(split=[int(len(tr_idx)), int(len(va_idx)), int(len(te_idx))],
                ...
                test_preds=dict(smiles=[g.smiles for g in te_g],
                                y=y_te.tolist(), mu=mu_te.tolist(), sigma=sd_te.tolist())), models

res_scaffold, models_scaf = run_experiment("SCAFFOLD split (strict)", scaffold_split_idx)
res_random, models_rand = run_experiment("RANDOM split (proposal-style)", random_split_idx)

out = dict(scaffold=res_scaffold, random=res_random)
json.dump(out, open(f"{RES}/tables/gnn_metrics_v2.json", "w"), indent=2)
torch.save({"models": [m.state_dict() for m, _ in models_scaf],
            "norm": [(ym, ys) for _, (ym, ys) in models_scaf],
            "node_dim": NODE_DIM}, f"{MOD}/bgnn_ensemble_scaffold.pt")
print("\nsaved results/tables/gnn_metrics_v2.json + models/bgnn_ensemble_scaffold.pt")
