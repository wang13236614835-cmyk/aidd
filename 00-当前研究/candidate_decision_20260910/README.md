...
> **2026-09-12 第五轮更新**：五候选（moracin N/formononetin/3'-methoxydaidzein/isorhamnetin/biochanin A）已完成独立审计并重新释放角色，见 [../fifth_round_20260912/第五轮_执行总览.md](../fifth_round_20260912/第五轮_执行总览.md)。要点：WT TRβ 区分门控失败（ROC-AUC 0.472）后 docking 分数不再参与排序；moracin N 转为 Nrf2/ferroptosis 机制重定位（B）；formononetin=疾病/机制锚点（C）；isorhamnetin=备用（B−，先过干扰门）；biochanin A=FXR 阳性对照（C）；3'-methoxydaidzein 降 D（亚微摩尔 NaV off-target，PMID 31262454）。本包其余内容为 2026-09-10 时点快照，其中与第五轮新口径冲突的表述以 [../fifth_round_20260912/历史错误与修正记录.md](../fifth_round_20260912/历史错误与修正记录.md) 为准。
