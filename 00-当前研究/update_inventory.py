...
def main():
 p=argparse.ArgumentParser();p.add_argument('--root',type=Path,default=Path(__file__).resolve().parents[1]);p.add_argument('--output',type=Path);a=p.parse_args();base=a.root.resolve();out=a.output or base/'00-当前研究/file_inventory.csv';out=out.resolve()
