> **2026-09-15 第五轮方向探索（当前最新）**：FASN Gate T1 已完成最终裁决，**Decision=FAIL**（51 assays 两两矩阵 1,275 对；共享≥15 的 4 对中，canonical/mirror 排除 1 对、protein layer 不匹配 2 对，真正可比独立 pair 仅 1 对；不构建 normalized dataset、不重跑模型）。FASN 收窄为 canonical local 方法学线；**Primary=Moracin N–NRF2/ferroptosis 肝细胞机制验证**；Secondary=FXR/Formononetin 依赖性桥接+Biochanin A QC；Reserve=THRβ reporter/Isorhamnetin/FASN直接酶实验/SCD1-DGAT2重资格化；NP54 FASN 定量榜、docking ranking、旧 GNN/CW-BCS/TOPSIS 正向结论和 3′-methoxydaidzein 当前创新位拒绝。GNN gate=`closed`，`candidate_release=false`。入口 [fifth_round_direction_exploration_20260915/EXECUTION_OVERVIEW.md](fifth_round_direction_exploration_20260915/EXECUTION_OVERVIEW.md)，裁决 [GATE_DECISION.json](fifth_round_direction_exploration_20260915/GATE_DECISION.json)。

> **2026-09-14 资产保留与 GNN 重规划**：当前阶段为 **pipeline revalidation / 资产保留优先**，不是候选已筛完。三个仓库角色不变：aidd 主库=唯一正式科研结论入口；Workbench=执行/审计只读镜像；hepato-gnn-screening=GNN 方法学习+历史复核+组员工作区。新增 [asset_preservation_and_gnn_replan_v1/执行总览.md](asset_preservation_and_gnn_replan_v1/执行总览.md) 与 [MASTER_ASSET_REGISTRY.csv](asset_preservation_and_gnn_replan_v1/MASTER_ASSET_REGISTRY.csv)：精选高价值资产全部存在并完成 hash 登记；旧数据、模型、split、日志、受体、docking 输出均保留，通过 VERIFIED/REQUALIFY/HISTORICAL/REJECTED 限定用途。GNN 仓库裁决为**小修即可、不改仓库名**，当前 `gated/closed`；`candidate_release=false` 维持。
